...
data (Ord a) => Memoized a b = Memoized (a -> b) (Map.Map a b)

data (Ord a, Ord b) => Memoized2 a b c = Memoized2 (a -> b  -> c) (Map.Map (a,b) c)

instance (Ord a) => Factory (Memoized a b) a b where

    facRun (Memoized f mp) x
...
instance (Ord a, Ord b) => Factory (Memoized2 a b c) (a,b) c where
